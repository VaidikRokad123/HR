...
- OTP-gated access to sensitive details (bank/payroll) — 6-digit OTP to admin email, 10-min TTL, 5-attempt limit
- Password hashing (bcryptjs)
- Pending: full RBAC middleware integration (foundation laid with permission constants)

---

## Tech Stack

**Backend:** Node.js, Express, MongoDB (Mongoose), JWT, bcryptjs, RabbitMQ (amqplib), Nodemailer + Google OAuth2, wkhtmltopdf, node-cron, multer, express-validator

**Frontend:** React 18, React Router v6, Axios, XLSX
...
**Windows:**
Download from https://wkhtmltopdf.org/downloads.html, then:
```
WKHTMLTOPDF_PATH=C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe
```
...
All routes are under `/api` (backend runs on port 5000).
