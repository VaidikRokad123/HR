import EmployeePersonal  from '../models/EmployeePersonalModel.js';
...
export async function runBirthdayCheck() {
  ...
  const employees = await EmployeePersonal.aggregate([
    {
      $match: {
  ...
  console.log(`[BirthdayChecker] Found ${employees.length} upcoming birthday(s) within 7 days`);
  ...
  for (const emp of employees) {
    ...
    const target = targetDates.find(t => t.month === emp.dobMonth && t.day === emp.dobDay);
