...
export function buildAppraisalLetterStructuredData(payload) {
  ...
  const closingParagraph1 = `We are sure you will make the best use of the opportunity offered to you and shall continue contributing to the success of our organization and fully justify the confidence placed in you by the management`;

  const closingParagraph2 = `A separate communication on the details of your salary revision is being sent to you.`;

  const wishLine = `Wish you all the best.`;
  const yoursTruly = `Yours Truly,`;

  const acceptanceLine = `As a token of this acceptance, request you to kindly execute this document and return one copy of the same to the HR Manager`;

  const employeeNameLine = `Employee’s Name: ${salutation} ${upperName}`;
    ...
    pages: [
      {
        ...
        paragraphs: [
          ...
          { id: 'closing1', content: closingParagraph1, type: 'paragraph' }
        ]
      },
      {
        pageNumber: 2,
        paragraphs: [
          { id: 'closing2', content: closingParagraph2, type: 'paragraph' },
          ...
          { id: 'footer1', content: employeeNameLine, type: 'footer' },
          { id: 'footer2', content: 'Employee’s Signature: _____________________', type: 'footer' },
          { id: 'footer3', content: 'Place: Ahmedabad, Gujarat, India', type: 'footer' }
        ]
      }
    ]
  };
